...
import { Mail, Phone, Building, Tag, Clock, MessageSquare, StickyNote, Lock, Globe, Users, Reply, Trash2, Edit2 } from "lucide-react";
...
export function ContactProfile({ contact }: ContactProfileProps) {
  ...
  const renderNote = (note: any, level = 0) => {
    ...
    return (
      <div key={note.id} className={`${level > 0 ? "ml-8 mt-2" : ""}`}>
          ...
          <div className="space-y-3">
            {messages && messages.length > 0 ? (
              messages.map((message: any) => (
                <div key={message.id} className="border-l-2 border-blue-600 pl-3 sm:pl-4 py-2">
                  <div className="flex items-center gap-1 sm:gap-2 mb-1 flex-wrap">
                    <Badge className="bg-blue-100 text-blue-800 text-xs">{message.channel}</Badge>
                    <Badge variant={message.direction === "INBOUND" ? "success" : "default"} className="text-xs">
                      {message.direction}
                    </Badge>
                    <span className="text-xs text-gray-500">
              ...
              <p className="text-xs sm:text-sm text-gray-500 text-center py-8">No activity yet</p>
            )}
          </div>
        )}
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {notes.length === 0 ? (
                ...
                notes.map((note: any) => (
                  <div key={note.id} className="bg-gray-50 rounded-lg p-3 sm:p-4">
                    <div className="flex items-start justify-between mb-2 gap-2">
                    ...
                    <p className="text-xs sm:text-sm text-gray-700 whitespace-pre-wrap break-words">{note.content}</p>
                  </div>
                ))
              )}
            </div>
